...
`MuM --bench <文件.md>` 分三段计时。它们的代价来源完全不同，只看"渲染"会给人错觉 ——
**滚动时真正反复发生的是排版，不是渲染**：
...
| **排版** | **1006 ms** | **9029 ms** | TextKit `ensureLayout`，**占 65%–78%** |
| 合计 | 1.54 s | 11.6 s | 对照：冷启动到窗口上屏 280 ms |

样本是合成文档（标题 + 段落 + 列表 + 引用 + 代码块 + 表格按真实比例混合），
1.00 MB / 5.00 MB，可用 `--bench` 复现。

**已知问题：5 MB 文档需要 9 秒排版，不可用。** 瓶颈是打开时对**整篇**做
`ensureLayout` —— 而 TextKit 默认是懒排版的，只排可见范围。当前实现里有两个地方
强制全量排版（一是打开时算文档高度以恢复阅读位置，二是 `scrollFraction()`）。

修法是改成按需排版 + 用 `layoutManager.estimatedHeight` 之类的方式算高度。
**留给后续版本**，0.3 先把基线记下来 —— 没有数字就没有优化。
