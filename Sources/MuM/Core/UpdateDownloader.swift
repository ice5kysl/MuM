...
enum UpdateDownloader {
    ...
    ///
    /// `then` 不能省：提示语说的是「安装盘**已**挂载」，只有挂载真的完成了才能说；
    /// 异步化之后写在下一行就会抢在挂载前面弹出来（这个回归 0.7.7 发出去才发现）。
    static func mountAndReveal(_ dmg: URL, then: @escaping () -> Void) {
        ExternalOpener.open(dmg, then: then)
    }
